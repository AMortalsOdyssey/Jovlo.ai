...
export function readableAuthError(error: unknown) {
  ...
  if (/already registered|user already exists|邮箱已注册/i.test(message)) return '该邮箱已注册，请直接登录。'
  ...
  if (/email.*rate.*limit|over_email_send_rate_limit/i.test(message)) return '验证邮件发送过于频繁，请稍后再试。'
